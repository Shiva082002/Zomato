// filterOptions.js

// Country filter options
export const countryOptions = [
    { value: "", label: "All" },
    { value: "1", label: "India" },
    { value: "14", label: "Australia" },
    { value: "30", label: "Brazil" },
    { value: "37", label: "Canada" },
    { value: "94", label: "Indonesia" },
    { value: "148", label: "New Zealand" },
    { value: "162", label: "Phillipines" },
    { value: "166", label: "Qatar" },
    { value: "184", label: "Singapore" },
    { value: "189", label: "South Africa" },
    { value: "191", label: "Sri Lanka" },
    { value: "208", label: "Turkey" },
    { value: "214", label: "UAE" },
    { value: "215", label: "United Kingdom" },
    { value: "216", label: "United States" },
  ];
  
  // Average spend filter options
  export const avgSpendOptions = [
    { value: "", label: "All" },
    { value: "500", label: "Up to ₹500" },
    { value: "1000", label: "Up to ₹1000" },
    { value: "1500", label: "Up to ₹1500" },
    { value: "2000", label: "Up to ₹2000" },
  ];
  
  // Cuisines filter options
  export const cuisinesOptions = [
    { value: "", label: "All" },
    { value: "american", label: "American" },
    { value: "asian", label: "Asian" },
    { value: "asian fusion", label: "Asian Fusion" },
    { value: "bakery", label: "Bakery" },
    { value: "beverages", label: "Beverages" },
    { value: "brazilian", label: "Brazilian" },
    { value: "burger", label: "Burger" },
    { value: "cafe", label: "Cafe" },
    { value: "cantonese", label: "Cantonese" },
    { value: "charcoal grill", label: "Charcoal Grill" },
    { value: "chettinad", label: "Chettinad" },
    { value: "chinese", label: "Chinese" },
    { value: "continental", label: "Continental" },
    { value: "contemporary", label: "Contemporary" },
    { value: "desserts", label: "Desserts" },
    { value: "dim sum", label: "Dim Sum" },
    { value: "european", label: "European" },
    { value: "fast food", label: "Fast Food" },
    { value: "finger food", label: "Finger Food" },
    { value: "french", label: "French" },
    { value: "fresh fish", label: "Fresh Fish" },
    { value: "goan", label: "Goan" },
    { value: "grill", label: "Grill" },
    { value: "ice cream", label: "Ice Cream" },
    { value: "indian", label: "Indian" },
    { value: "indonesian", label: "Indonesian" },
    { value: "italian", label: "Italian" },
    { value: "japanese", label: "Japanese" },
    { value: "kiwi", label: "Kiwi" },
    { value: "latin American", label: "Latin American" },
    { value: "lebanese", label: "Lebanese" },
    { value: "malay", label: "Malay" },
    { value: "mangalorean", label: "Mangalorean" },
    { value: "mediterranean", label: "Mediterranean" },
    { value: "mexican", label: "Mexican" },
    { value: "mughlai", label: "Mughlai" },
    { value: "north indian", label: "North Indian" },
    { value: "parsi", label: "Parsi" },
    { value: "peranakan", label: "Peranakan" },
    { value: "pizza", label: "Pizza" },
    { value: "pakistani", label: "Pakistani" },
    { value: "seafood", label: "Seafood" },
    { value: "singaporean", label: "Singaporean" },
    { value: "south african", label: "South African" },
    { value: "south indian", label: "South Indian" },
    { value: "spanish", label: "Spanish" },
    { value: "steak", label: "Steak" },
    { value: "street food", label: "Street Food" },
    { value: "sushi", label: "Sushi" },
    { value: "tapas", label: "Tapas" },
    { value: "tex-mex", label: "Tex-Mex" },
    { value: "thai", label: "Thai" },
    { value: "western", label: "Western" },
  ];


export const menu_item_map = {
    "american": [
        "apple pie",
        "baby back ribs",
        "beef carpaccio",
        "chocolate cake",
        "chocolate mousse",
        "chicken wings",
        "clam chowder",
        "club sandwich",
        "crab cakes",
        "foie gras",
        "hamburger",
        "pulled pork sandwich",
        
        "waffles"
    ],
    "arabian": [
        "hummus",
        "falafel"
    ],
    "asian": [
        "bibimbap",
        "dumplings",
        "edamame",
        "gyoza",
        "miso soup",
        "pho",
        "ramen",
        "spring rolls",
        "takoyaki",
        "sushi"
    ],
    "bakery": [
        "baklava",
        "beignets",
        "cannoli",
        "cup cakes",
        "donuts",
        "macarons",
        "panna cotta",
        "red velvet cake"
    ],
    "brazilian": [
        "none"
    ],
    "burger": [
        "none"
    ],
    "cafe": [
        "none"
    ],
    "charcoal grill": [
        "none"
    ],
    "chinese": [
        "chicken curry",
        "chicken quesadilla",
        "fried rice",
        "hot and sour soup",
        "peking duck",
        "spring rolls"
    ],
    "continental": [
        "caesar salad",
        "crepe",
        "filet mignon",
        "garlic bread",
        "gnocchi",
        "grilled salmon",
        "paella",
        "ravioli",
        "risotto",
        "tiramisu"
    ],
    "contemporary": [
        "none"
    ],
    "desserts": [
        "churros",
        "creme brulee",
        "cup cakes",
        "donuts",
        "frozen yogurt",
        "macarons",
        "panna cotta",
        "red velvet cake",
        "strawberry shortcake",
        "ice cream"
    ],
    "filipino": [
        "none"
    ],
    "finger food": [
        "guacamole",
        "nachos",
        "onion rings"
    ],
    "french": [
        "french fries",
        "french onion soup",
        "french toast",
        "foie gras",
        "quiche"
    ],
    "fusion": [
        "none"
    ],
    "greek": [
        "greek salad"
    ],
    "healthy food": [
        "none"
    ],
    "international": [
        "none"
    ],
    "italian": [
        "bruschetta",
        "caprese salad",
        "lasagna",
        "macaroni and cheese",
        "ravioli",
        "tiramisu"
    ],
    "izgara": [
        "none"
    ],
    "japanese": [
        "edamame",
        "miso soup",
        "sushi",
        "takoyaki"
    ],
    "kebab": [
        "none"
    ],
    "korean": [
        "bibimbap"
    ],
    "kiwi": [
        "none"
    ],
    "lebanese": [
    ],
    "malaysian": [
        "none"
    ],
    "mediterranean": [
        "greek salad",   
     ],
    "mexican": [
        "breakfast burrito",
        "tacos"
    ],
    "modern indian": [
        "none"
    ],
    "mughlai": [
        "none"
    ],
    "north indian": [
        "chicken curry",
        "chicken quesadilla",
        "samosa"
    ],
    "pakistani": [
        "none"
    ],
    "pizza": [
        "pizza",
        "pizza"
    ],
    "pub food": [
        "none"
    ],
    "rajasthani": [
        "none"
    ],
    "south african": [
        "none"
    ],
    "south indian": [
        "dosa"
    ],
    "steak": [
        "filet mignon",
        "prime rib",
        "steak"
    ],
    "street food": [
        "churros",
        "hot dog",
        "tacos"
    ],
    "taiwanese": [
        "none"
    ],
    "tapas": [
        "none"
    ],
    "thai": [
        "pad thai",
        "tom yum soup"
    ],
    "tibetan": [
        "none"
    ],
    "western": [
        "beef tartare",
        "filet mignon"
    ],
    "world cuisine": [
        "none"
    ]
}
